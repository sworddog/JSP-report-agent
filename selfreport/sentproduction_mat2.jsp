...
<%
    ...
    if (isExport) {
        ...
        response.setHeader("Content-Disposition", "attachment; filename=" +
            URLEncoder.encode("蛟龙颜旭发料统计报表_物料2.xls", "UTF-8"));
    }
%>
<% if (!isExport) { %>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>蛟龙 / 颜旭 发料统计报表（物料编码2开头）</title>
    <style>
...
<%
    ...
    try {
        ...
        String sql2 = "SELECT [FENTRYID],[单据编号],[采购日期],[供应商],[采购订单行号],[规格型号],[采购数量],[交货日期],[备注],[剩余未发数量],[物料编码] " +
                      "FROM VIEW_TO_OA_CGDD WHERE [物料编码] LIKE '2%' ORDER BY [采购日期]";
